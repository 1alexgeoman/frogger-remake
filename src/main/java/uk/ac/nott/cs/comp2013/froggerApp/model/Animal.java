package uk.ac.nott.cs.comp2013.froggerApp.model;

import java.util.ArrayList;

import javafx.event.EventHandler;

import javafx.scene.image.Image;
import javafx.scene.input.KeyCode;
import javafx.scene.input.KeyEvent;

/**
 * Represents the Frogger game character.
 */
public class Animal extends Actor {
	Image imgW1;
	Image imgA1;
	Image imgS1;
	Image imgD1;
	Image imgW2;
	Image imgA2;
	Image imgS2;
	Image imgD2;
	int points = 0;
	int end = 0;
	private boolean second = false;
	boolean noMove = false;
	double movement = 26.6;
	double movementX = 21.2;
	int imgSize = 40;
	boolean carDeath = false;
	boolean waterDeath = false;
	boolean stop = false;
	boolean changeScore = false;
	private int lives = 5;
	int carD = 0;
	double w = 800;
	ArrayList<End> inter = new ArrayList<End>();
	/**
	 * Constructs the Animal object.
	 * @param imageLink initial image path
	 */
	public Animal(String imageLink) {
		setImage(new Image(imageLink, imgSize, imgSize, true, true));
		setX(300);
		setY(679.8+movement);
		imgW1 = new Image("/froggerUp.png", imgSize, imgSize, true, true);
		imgA1 = new Image("/froggerLeft.png", imgSize, imgSize, true, true);
		imgS1 = new Image("/froggerDown.png", imgSize, imgSize, true, true);
		imgD1 = new Image("/froggerRight.png", imgSize, imgSize, true, true);
		imgW2 = new Image("/froggerUpJump.png", imgSize, imgSize, true, true);
		imgA2 = new Image("/froggerLeftJump.png", imgSize, imgSize, true, true);
		imgS2 = new Image("/froggerDownJump.png", imgSize, imgSize, true, true);
		imgD2 = new Image("/froggerRightJump.png", imgSize, imgSize, true, true);
		setOnKeyPressed(new EventHandler<KeyEvent>() {
			public void handle(KeyEvent event){
				if (noMove) {
				}
				else {
				if (second) {
					if (event.getCode() == KeyCode.W) {
		                move(0, -movement);
		                changeScore = false;
		                setImage(imgW1);
		                second = false;

		            }
		            else if (event.getCode() == KeyCode.A) {
		            	 move(-movementX, 0);
		            	 setImage(imgA1);
		            	 second = false;
		            }
		            else if (event.getCode() == KeyCode.S) {
		            	 move(0, movement);
		            	 setImage(imgS1);
		            	 second = false;
		            }
		            else if (event.getCode() == KeyCode.D) {
		            	 move(movementX, 0);
		            	 setImage(imgD1);
		            	 second = false;
		            }
				}
				else if (event.getCode() == KeyCode.W) {
	                move(0, -movement);
	                setImage(imgW2);
	                second = true;
					System.out.println(getY());
	            }
	            else if (event.getCode() == KeyCode.A) {
	            	 move(-movementX, 0);
	            	 setImage(imgA2);
	            	 second = true;
	            }
	            else if (event.getCode() == KeyCode.S) {
	            	 move(0, movement);
	            	 setImage(imgS2);
	            	 second = true;
	            }
	            else if (event.getCode() == KeyCode.D || event.getCode() == KeyCode.RIGHT) {
	            	 move(movementX, 0);
	            	 setImage(imgD2);
	            	 second = true;
	            }
	        }
			}
		});
		setOnKeyReleased(new EventHandler<KeyEvent>() {
			public void handle(KeyEvent event) {
				if (noMove) {}
				else {
				if (event.getCode() == KeyCode.W) {
					if (getY() < w) {
						changeScore = true;
						w = getY();
						points+=10;
					}
	                move(0, -movement);
	                setImage(imgW1);
	                second = false;
	            }
	            else if (event.getCode() == KeyCode.A) {
	            	 move(-movementX, 0);
	            	 setImage(imgA1);
	            	 second = false;
	            }
	            else if (event.getCode() == KeyCode.S) {
	            	 move(0, movement);
	            	 setImage(imgS1);
	            	 second = false;
	            }
	            else if (event.getCode() == KeyCode.D) {
	            	 move(movementX, 0);
	            	 setImage(imgD1);
	            	 second = false;
	            }
	        }
			}

		});
	}
	/**
	 * Executes game logic each frame.
	 * @param now current timestamp
	 */
	@Override
	public void act(long now) {
		if (getY()<0 || getY()>734) {
			setX(300);
			setY(679.8+movement);
		}
		if (getX()<0) {
			move(movement*2, 0);
		}
		if (carDeath) {
			noMove = true;
			if ((now)% 11 ==0) {
				carD++;
			}
			switch (carD) {
				case 1 -> setImage(new Image("/cardeath1.png", imgSize, imgSize, true, true));
				case 2 -> setImage(new Image("/cardeath2.png", imgSize, imgSize, true, true));
				case 3 -> setImage(new Image("/cardeath3.png", imgSize, imgSize, true, true));
				case 4 -> resetAfterDeath();
			}
		}
		if (waterDeath) {
			noMove = true;
			if ((now)% 11 ==0) {
				carD++;
			}
			switch (carD) {
				case 1 -> setImage(new Image("/waterdeath1.png", imgSize, imgSize, true, true));
				case 2 -> setImage(new Image("/waterdeath2.png", imgSize, imgSize, true, true));
				case 3 -> setImage(new Image("/waterdeath3.png", imgSize, imgSize, true, true));
				case 4 -> setImage(new Image("/waterdeath4.png", imgSize, imgSize, true, true));
				case 5 -> resetAfterDeath();
			}
		}
		
		if (getX()>600) {
			move(-movement*2, 0);
		}
		checkIntersection();
	}
	/**
	 * Resets position and lives after death.
	 */
	private void resetAfterDeath() {
		setX(300);
		loseLife();
		setY(679.8 + movement);
		carDeath = false;
		waterDeath = false;
		carD = 0;
		setImage(new Image("/froggerUp.png", imgSize, imgSize, true, true));
		noMove = false;
		if (points > 50) {
			points -= 50;
			changeScore = true;
		}
	}
	/**
	 * Returns remaining lives.
	 * @return remaining lives
	 */
	public int getLives() {
		return lives;
	}

	/**
	 * Handles loss of a life.
	 */
	private void loseLife() {
		lives--;
		if (lives == 0) {
			stop = true; // Signal the game to end
		}
	}
	/**
	 * Checks for intersections with game objects.
	 */
	private void checkIntersection(){
		if (getIntersectingObjects(Obstacle.class).size() >= 1) {
			carDeath = true;
		}
		if (getX() == 240 && getY() == 82) {
			stop = true;
		}
		if (getIntersectingObjects(Log.class).size() >= 1 && !noMove) {
			if(getIntersectingObjects(Log.class).get(0).getLeft())
				move(-2,0);
			else
				move (.75,0);
		}
		else if (getIntersectingObjects(Turtle.class).size() >= 1 && !noMove) {
			move(-1,0);
		}
		else if (getIntersectingObjects(WetTurtle.class).size() >= 1) {
			if (getIntersectingObjects(WetTurtle.class).get(0).isSunk()) {
				waterDeath = true;
			} else {
				move(-1,0);
			}
		}
		else if (getIntersectingObjects(End.class).size() >= 1) {
			inter = (ArrayList<End>) getIntersectingObjects(End.class);
			if (getIntersectingObjects(End.class).get(0).isActivated()) {
				end--;
				points-=50;
			}
			points+=50;
			changeScore = true;
			w=800;
			getIntersectingObjects(End.class).get(0).setEnd();
			end++;
			lives = 5;
			setX(300);
			setY(679.8+movement);
		}
		else if (getY()<413){
			waterDeath = true;
		}
	}

	/**
	 * Returns true if the game is over.
	 * @return game over status
	 */
	public boolean getStop() {
		return end==5;
	}

	/**
	 * Returns current points.
	 * @return current score
	 */
	public int getPoints() {
		return points;
	}

	/**
	 * Checks and resets score change flag.
	 * @return true if score changed
	 */
	public boolean changeScore() {
		if (changeScore) {
			changeScore = false;
			return true;
		}
		return false;
		
	}
	/**
	 * Checks if water death occurred.
	 * @return true if water death
	 */
	public boolean isWaterDeath() {
		return waterDeath;
	}

	/**
	 * Checks if car death occurred.
	 *
	 * @return true if car death
	 */
	public boolean isCarDeath() {
		return carDeath;
	}

}
